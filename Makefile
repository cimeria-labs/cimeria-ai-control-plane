...
stop:
	...
	@case "$(DATABASE_URL)" in \
		""|*@localhost:*|*@localhost/*|*@127.0.0.1:*|*@127.0.0.1/*|*@\[::1\]:*|*@\[::1\]/*) \
			echo "[ok] App processes stopped. Shared PostgreSQL is still running on localhost:$(POSTGRES_PORT)." ;; \
		*) \
			echo "[ok] App processes stopped. Remote PostgreSQL was not affected." ;; \
	esac
...
daemon:
	@$(MAKE) cim MULTICA_ARGS="daemon restart --profile local"

cli:
	@$(MAKE) cim MULTICA_ARGS="$(MULTICA_ARGS)"

cim:
	cd server && go run ./cmd/cim $(MULTICA_ARGS)
...
build:
	...
	cd server && go build -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" -o bin/cim ./cmd/cim
