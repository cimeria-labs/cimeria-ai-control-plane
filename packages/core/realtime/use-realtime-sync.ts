...
export function useRealtimeSync(
  ...
  useEffect(() => {
    ...
    const refreshMap: Record<string, () => void> = {
      ...
      email: () => {
        const wsId = getCurrentWsId();
        if (wsId) qc.invalidateQueries({ queryKey: leadKeys.all(wsId) });
      },
      agent: () => {
        const wsId = getCurrentWsId();
    ...
    const unsubAny = ws.onAny((msg) => {
      ...
      const prefix = msg.type.includes(":")
        ? msg.type.split(":")[0] ?? ""
        : msg.type.split(".")[0] ?? "";
  ...
  // Browser tabs can resume after sleep without receiving a clean reconnect.
  // Refresh the central workspace screens when the tab becomes visible again.
  useEffect(() => {
