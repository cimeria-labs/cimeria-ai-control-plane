...
export class ApiClient {
  ...
  // Leads
  async listLeads(params?: ListLeadsParams): Promise<ListLeadsResponse> {
    const search = new URLSearchParams();
    if (params?.limit) search.set("limit", String(params.limit));
    if (params?.offset) search.set("offset", String(params.offset));
    if (params?.status) search.set("status", params.status);
    return this.fetch(`/api/leads?${search}`);
  }

  async getLead(id: string): Promise<Lead> {
    return this.fetch(`/api/leads/${id}`);
  }
  ...
  async importLeadsCsv(csv: string): Promise<ImportLeadsResponse> {
    return this.fetch("/api/leads/import-csv", {
      method: "POST",
  ...
  async listLeadScoreRules(): Promise<LeadScoreRule[]> {
    return this.fetch("/api/lead-score-rules");
  }

  async upsertLeadScoreRule(data: UpsertLeadScoreRuleRequest): Promise<LeadScoreRule> {
    return this.fetch("/api/lead-score-rules", {
      method: "PUT",
      body: JSON.stringify(data),
    });
  ...
  async deleteLeadScoreRule(eventType: string): Promise<void> {
    await this.fetch(`/api/lead-score-rules/${encodeURIComponent(eventType)}`, {
      method: "DELETE",
    });
  }
  ...
  /** Trigger an autopilot via webhook token (public, no auth required). */
  async triggerWebhook(token: string, payload?: unknown): Promise<AutopilotRun> {
    const res = await fetch(`${this.baseUrl}/api/webhooks/${token}`, {
      method: "POST",
