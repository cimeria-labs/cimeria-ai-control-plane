...
const cliConfigHomeEnv = "MULTICA_CONFIG_HOME"
...
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(base, "profiles", profile, "config.json"), nil
}
...
func cliConfigBaseDir() (string, error) {
	if override := os.Getenv(cliConfigHomeEnv); override != "" {
		return override, nil
	}
