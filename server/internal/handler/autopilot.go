...
// HandleWebhook receives an inbound webhook request, looks up the trigger by
// token, and dispatches the autopilot run with the request body as payload.
func (h *Handler) HandleWebhook(w http.ResponseWriter, r *http.Request) {
	...
	if token == "" {
		writeError(w, http.StatusBadRequest, "missing webhook token")
		return
	}

	row, err := h.Queries.GetAutopilotTriggerByWebhookToken(r.Context(), pgtype.Text{String: token, Valid: true})
	if err != nil {
		writeError(w, http.StatusNotFound, "webhook not found")
		return
	}

	if row.AutopilotStatus != "active" {
		writeError(w, http.StatusBadRequest, "autopilot is not active")
		return
	}

	autopilot, err := h.Queries.GetAutopilot(r.Context(), row.AutopilotID)
	if err != nil {
		writeError(w, http.StatusNotFound, "autopilot not found")
		return
	}
	...
	run, err := h.AutopilotService.DispatchAutopilot(r.Context(), autopilot, row.ID, "webhook", body)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to dispatch autopilot: "+err.Error())
		return
	}

	// Update last_fired_at on the trigger.
	h.Queries.UpdateTriggerLastFired(r.Context(), row.ID)

	writeJSON(w, http.StatusOK, runToResponse(*run))
}

// generateWebhookToken creates a cryptographically random 32-byte hex token.
