...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	b.WriteString("- `multica email send --to <email1,email2> --subject \"...\" --body \"...\" [--html \"...\"]` — Send an email from the workspace\n\n")
	...
	if ctx.ChatSessionID != "" {
		...
		doneStatus := "done"
		if ctx.AgentHandoffMode == "manual" {
			doneStatus = "in_review"
		}
		fmt.Fprintf(&b, "5. When done, run `multica issue status %s %s`\n", ctx.IssueID, doneStatus)
