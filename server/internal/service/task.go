...
// Register subscribes the task service to domain events so it can enqueue
// agent tasks when issues are created outside the HTTP handler (e.g. by the
// SDR engine).
func (s *TaskService) Register() {
	s.Bus.Subscribe(protocol.EventIssueCreated, func(e events.Event) {
		issueID := extractIssueIDFromPayload(e.Payload)
		if issueID == "" {
			return
		}
		...
		if err != nil {
			slog.Warn("task listener: failed to load issue", "issue_id", issueID, "error", err)
			return
		}
		...
		if _, err := s.EnqueueTaskForIssue(ctx, issue); err != nil {
			slog.Warn("task listener: failed to enqueue task", "issue_id", issueID, "error", err)
		}
	})
}

func extractIssueIDFromPayload(payload any) string {
