...
// SendGenericEmail sends an arbitrary email on behalf of the workspace.
// to is a list of recipient email addresses (max 10).
// subject is the email subject (will be sanitized).
// textBody is the plain-text body (required if htmlBody is empty).
// htmlBody is the HTML body (optional; if set, textBody is used as text fallback).
// Returns the Resend email ID, or "dev-mode" when running without RESEND_API_KEY.
func (s *EmailService) SendGenericEmail(to []string, subject, textBody, htmlBody string) (string, error) {
	subject = sanitizeSubjectField(subject)

	if s.client == nil {
		fmt.Printf("[DEV] Generic email to %v: subject=%q body_len=%d html_len=%d\n", to, subject, len(textBody), len(htmlBody))
		return "dev-mode", nil
	}
	...
	result, err := s.client.Emails.Send(req)
	if err != nil {
