...
const getAutopilotTriggerByWebhookToken = `-- name: GetAutopilotTriggerByWebhookToken :one

SELECT t.id, t.autopilot_id, t.kind, t.enabled, t.cron_expression, t.timezone, t.next_run_at, t.webhook_token, t.label, t.last_fired_at, t.created_at, t.updated_at, a.workspace_id AS autopilot_workspace_id, a.status AS autopilot_status, a.assignee_id
...
type GetAutopilotTriggerByWebhookTokenRow struct {
	ID                   pgtype.UUID        `json:"id"`
	AutopilotID          pgtype.UUID        `json:"autopilot_id"`
	Kind                 string             `json:"kind"`
	Enabled              bool               `json:"enabled"`
...
// =====================
func (q *Queries) GetAutopilotTriggerByWebhookToken(ctx context.Context, webhookToken pgtype.Text) (GetAutopilotTriggerByWebhookTokenRow, error) {
	row := q.db.QueryRow(ctx, getAutopilotTriggerByWebhookToken, webhookToken)
	var i GetAutopilotTriggerByWebhookTokenRow
	err := row.Scan(
		&i.ID,
		&i.AutopilotID,
		&i.Kind,
...
const updateTriggerLastFired = `-- name: UpdateTriggerLastFired :exec
UPDATE autopilot_trigger
SET last_fired_at = now(), updated_at = now()
WHERE id = $1
`

func (q *Queries) UpdateTriggerLastFired(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, updateTriggerLastFired, id)
	return err
}
