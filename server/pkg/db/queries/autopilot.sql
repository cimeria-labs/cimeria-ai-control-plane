...
-- name: GetAutopilotTriggerByWebhookToken :one
SELECT t.*, a.workspace_id AS autopilot_workspace_id, a.status AS autopilot_status, a.assignee_id
FROM autopilot_trigger t
JOIN autopilot a ON t.autopilot_id = a.id
WHERE t.webhook_token = $1
  AND t.kind = 'webhook'
  AND t.enabled = true;

-- name: UpdateTriggerLastFired :exec
UPDATE autopilot_trigger
