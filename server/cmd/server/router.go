...
func NewRouter(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus) chi.Router {
	...
	// Email tracking (public — recipients are not authenticated users)
	r.Get("/track/pixel/{id}", h.TrackPixel)
	...
	// Webhook (public, token-authenticated)
	r.Post("/api/webhooks/{token}", h.HandleWebhook)

	// Resend webhook (public, Resend-signed)
	r.Post("/api/webhooks/resend", h.HandleResendWebhook)
	...
	r.Group(func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			// Email
			r.Post("/api/emails/send", h.SendEmail)

			// Leads
			r.Route("/api/leads", func(r chi.Router) {
				r.Get("/", h.ListLeads)
				r.Post("/", h.CreateLead)
				r.Post("/import-csv", h.ImportLeadsCSV)
				r.Post("/bulk-curate", h.BulkCurateLeads)
				r.Route("/{id}", func(r chi.Router) {
					r.Get("/", h.GetLead)
					r.Patch("/", h.UpdateLead)
					r.Post("/evaluate-rules", h.EvaluateCuratorRules)
				})
			})

			// Lead sources
			r.Route("/api/lead-sources", func(r chi.Router) {
				r.Get("/", h.ListLeadSources)
				r.Post("/", h.CreateLeadSource)
				r.Route("/{id}", func(r chi.Router) {
					r.Get("/", h.GetLeadSource)
					r.Patch("/", h.UpdateLeadSource)
					r.Delete("/", h.DeleteLeadSource)
				})
			})

			// Lead import batches
			r.Route("/api/lead-import-batches", func(r chi.Router) {
				r.Get("/", h.ListLeadImportBatches)
				r.Post("/", h.CreateLeadImportBatch)
				r.Route("/{id}", func(r chi.Router) {
			...
			// Lead curator rules
			r.Route("/api/lead-curator-rules", func(r chi.Router) {
				r.Get("/", h.ListLeadCuratorRules)
				r.Post("/", h.CreateLeadCuratorRule)
				r.Route("/{id}", func(r chi.Router) {
					r.Get("/", h.GetLeadCuratorRule)
					r.Patch("/", h.UpdateLeadCuratorRule)
					r.Delete("/", h.DeleteLeadCuratorRule)
				})
			})

			// Lead score rules
			r.Route("/api/lead-score-rules", func(r chi.Router) {
				r.Get("/", h.ListLeadScoreRules)
				r.Put("/", h.UpsertLeadScoreRule)
				r.Patch("/", h.UpsertLeadScoreRule)
				r.Put("/{eventType}", h.UpsertLeadScoreRule)
				r.Patch("/{eventType}", h.UpsertLeadScoreRule)
				r.Delete("/{eventType}", h.DeleteLeadScoreRule)
			})
